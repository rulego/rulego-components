...
type ClientNodeConfiguration struct {
	...
	OpType string
	// 过滤条件，支持${}占位符。示例：{"age": {"$gte": 18}}
	Filter string
	//更新/插入文档。支持${}占位符。示例：{"name":"test","age":18}
	Doc string
	// 是否只操作一条数据
	One bool
}
...
type ClientNode struct {
	...
	// 过滤
	FilterTemplate str.Template
	// 文档
	DocTemplate str.Template
}
...
func (x *ClientNode) Init(ruleConfig types.Config, configuration types.Configuration) error {
	...
	if x.Config.Filter != "" {
		x.FilterTemplate = str.NewTemplate(strings.TrimSpace(x.Config.Filter))
	}
	if x.Config.Doc != "" {
		x.DocTemplate = str.NewTemplate(strings.TrimSpace(x.Config.Doc))
	}
	// 初始化客户端
...
func (x *ClientNode) toBsonM(evn map[string]interface{}, template str.Template) (interface{}, error) {
	v := template.Execute(evn)
	if v == "" {
		return nil, nil
	}
	var out interface{}
...
func (x *ClientNode) toBsonMList(evn map[string]interface{}, template str.Template) ([]interface{}, error) {
	v := template.Execute(evn)
	if v == "" {
		return nil, nil
	}
